...
features_join = ch_df |>
  ...
  group_by(VALUE) |>
  summarise(ALL_FEATURES=paste(feature_lookup[Features],collapse="; "),
            C1=na_if(paste(keep(feature_lookup[Features], feature_lookup[Features] %in% lookup[lookup$C1==1,]$Feature),collapse="; "),""),
            C2=na_if(paste(keep(feature_lookup[Features], feature_lookup[Features] %in% lookup[lookup$C2==1,]$Feature),collapse="; "),""),
            C3=na_if(paste(keep(feature_lookup[Features], feature_lookup[Features] %in% lookup[lookup$C3==1,]$Feature),collapse="; "),""),
            C4=na_if(paste(keep(feature_lookup[Features], feature_lookup[Features] %in% lookup[lookup$C4==1,]$Feature),collapse="; "),""),
            C5=na_if(paste(keep(feature_lookup[Features], feature_lookup[Features] %in% lookup[lookup$C5==1,]$Feature),collapse="; "),""))
